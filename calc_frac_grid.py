...
def calcFracGrid(OBS,hisfile,onlyVertical=False,method='cubic'):
    '''
    ...
    Adepth=grid.z_r
    if  not onlyVertical:
        print 'recalculating horizontal grid fractions'
        OBS.Xgrid,OBS.Ygrid=obs_ijpos(hisfile,OBS.lon,OBS.lat,'r')
    popindex=np.unique(np.array([np.argwhere(OBS.Xgrid<0),np.argwhere(OBS.Ygrid<0)]).flatten())
    print len(popindex)
