...
def plot_obs(S,hisfile,obstype=6,depthmin=None, depthmax=None,timemin=None,timemax=None,Vmin=None,Vmax=None,fileout=None):
    ...
    OBS=OBS[np.where(OBS.type==obstype)]
    if not OBS.Ndatum:
        print('No observations of type ',obstype,' in dataset')
        return
    ...
    if timemin is not None:
        ...
        if not OBS.Ndatum:
            print('No observations of type ',obstype,' in specified time range')
            return
    ...
    if depthmin is not None:
        ...
        if not OBS.Ndatum:
            print('No observations of type ',obstype,' in specified depth range')
            return
    ...
    im=plt.scatter(OBS.Xgrid,OBS.Ygrid,c=OBS.value,edgecolor='None',cmap=obstypes[obstype]['cmap'])
